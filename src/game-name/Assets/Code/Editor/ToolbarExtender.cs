...
namespace Code.Editor
{
    ...
    public static class ToolbarExtender
    {
        ...
        private const int ALLOCATED_SIZE = 8;

        private static readonly Action[] leftToolbarGUI = new Action[ALLOCATED_SIZE];
        private static readonly Action[] rightToolbarGUI = new Action[ALLOCATED_SIZE];
	    ...
	    {
		    if (order < 0) 
			    Debug.LogError("Order should be greater or equal to 0");
		    
		    rightToolbarGUI[order] = right;
	    }
	    
	    public static void RegisterLeftEntry(Action left, int order)
	    {
		    if (order < 0) 
			    Debug.LogError("Order should be greater or equal to 0");
		    
		    leftToolbarGUI[order] = left;
	    }
        ...
        private static void GUILeft()
        {
            ...
            for (var i = 0; i < leftToolbarGUI.Length; i++)
            {
	            if (leftToolbarGUI[i] != null)
	            {
		            leftToolbarGUI[i]();
	            }
            }
        ...
        private static void GUIRight()
        {
            ...
            for (var i = 0; i < rightToolbarGUI.Length; i++)
            {
	            if (rightToolbarGUI[i] != null)
	            {
		            rightToolbarGUI[i]();
	            }
            }
